...
title(s2,sprintf("Expectation (%d iterations) of the variance and the bias\n for estimation of $\\mu$  by an open multi-agent system.\n $n = %d$, $\\lambda_c =$ %1.2f and $\\lambda_r =$%1.2f",nreal,n,lci,lri),'Interpreter','Latex');
...
% print(f1,'plots/Gossip/gossip_vs_best_mse_time','-dpdf'); % then print it
% set(f2,'PaperSize',[15 11]); %set the paper size to what you want  
% print(f2,'plots/Gossip/gossip_vs_best__VB_time','-dpdf'); % then print it
